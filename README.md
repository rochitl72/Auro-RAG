...
This section provides visual documentation of the AuroRag system interface, showcasing the complete 4-phase agentic workflow in action.

### 🚀 Final System Result

![System Execution Workflow](images/WhatsApp%20Image%202026-02-06%20at%2010.25.33.jpeg)

*Figure: This image illustrates the full agentic loop. It highlights the transparency of the 'thought process' where the system explains how it mapped the medical terminology to the SQL schema and returned the final patient count from the EMR Data Lake.*

**What's Shown:**
- Complete query execution workflow from natural language input to final results
- Real-time visualization of all four agent phases (Receptionist → Librarian → Engineer → Inspector)
- Transparent "thought process" display showing how medical terminology is mapped to database schema
- Final patient count results retrieved from the EMR Data Lake
- Natural language explanation generated by the LLM

**Key Features Demonstrated:**
- End-to-end query processing transparency
- Medical domain-specific terminology understanding
- SQL schema mapping visualization
- Result summarization with supporting data

---

### 🔍 Intelligent Schema Selection (The Librarian)

![Librarian Selection Process](images/WhatsApp%20Image%202026-02-06%20at%2010.36.56.jpeg)

*Figure: Demonstrating the 'Lazy Loading' technique. The system autonomously selects a subset of columns (Schema Linking) to provide the LLM with focused context, preventing hallucinations and reducing compute costs.*

**What's Shown:**
- The **Librarian Agent** in action, performing intelligent schema linking
- Lazy loading technique where only relevant columns are selected from the 31 available columns
- Autonomous column selection based on query context and semantic matching
- Focused context provision to the LLM to prevent hallucinations
- Cost optimization through selective column retrieval

**Key Features Demonstrated:**
- **Lazy Loading**: Only loading relevant schema information instead of all 31 columns
- **Schema Linking**: Intelligent matching of query intent to database columns
- **Hallucination Prevention**: Focused context reduces LLM errors
- **Cost Efficiency**: Reduced token usage by selecting only necessary columns
- **Autonomous Selection**: LLM-driven column relevance scoring

**Technical Process:**
1. Librarian receives user query and execution plan
2. Analyzes all 31 available columns from schema description
3. LLM selects top 8-10 most relevant columns based on:
   - Query keyword matching
   - Semantic similarity
   - Plan requirements
4. Returns ordered list with similarity scores
5. Provides focused context to Engineer agent

---

### ✅ Data Validation & Output (The Inspector)

![Final Data Output](images/WhatsApp%20Image%202026-02-06%20at%2010.37.29.jpeg)

*Figure: The final output screen. The Inspector agent ensures the SQL results are accurate before the system generates a human-readable summary and displays the supporting data table.*

**What's Shown:**
- **Inspector Agent** performing final validation and result formatting
- SQL query execution results displayed in a structured table format
- Data validation ensuring accuracy before presentation
- Human-readable summary generated by the LLM
- Supporting data table showing detailed query results
- Complete pipeline visualization with all phases marked as complete

**Key Features Demonstrated:**
- **Result Validation**: Inspector verifies SQL execution success
- **Data Accuracy**: Ensures results match query intent
- **Natural Language Summary**: LLM-generated human-readable explanation
- **Structured Data Display**: Formatted table with scrollable results
- **Complete Workflow**: All four phases successfully completed

**Technical Process:**
1. Inspector executes SQL query on in-memory SQLite database
2. Validates query execution (catches errors, triggers retries if needed)
3. Formats results into DataFrame
4. Generates natural language explanation using LLM
5. Displays both summary and detailed data table
6. Marks all pipeline phases as complete

---

### 🛠️ Backend Architecture & Local Stack

![System Configuration Status](images/WhatsApp%20Image%202026-02-06%20at%2011.23.18.jpeg)

*Figure: The System Health panel. This view confirms the local deployment of the DeepSeek-R1 model via Ollama and the successful indexing of the Ophthalmology dataset into the vector store for RAG operations.*

**What's Shown:**
- **System Health Dashboard** displaying backend architecture status
- Local LLM deployment confirmation (DeepSeek-R1 model via Ollama)
- Ollama service status and availability
- Dataset indexing status showing successful vector store creation
- Ophthalmology dataset integration confirmation
- RAG (Retrieval-Augmented Generation) operations readiness
- Real-time system statistics (rows, columns, tables)

**Key Features Demonstrated:**
- **Local Deployment**: All processing happens locally, ensuring data privacy
- **Ollama Integration**: Local LLM inference server status
- **Vector Store**: Successful indexing of medical dataset for RAG
- **System Health Monitoring**: Real-time status of all components
- **Data Statistics**: Live metrics from the actual dataset

**Technical Components:**
- **Ollama Service**: Running locally on port 11434
- **LLM Model**: DeepSeek-R1 (or Llama 3.1 8B) loaded and ready
- **Vector Store**: Schema embeddings generated and indexed
- **Dataset**: 492 rows, 31 columns successfully loaded
- **RAG Pipeline**: Ready for retrieval-augmented generation operations

---

### Workflow Summary

These screenshots collectively demonstrate the complete **4-phase agentic workflow**:

1. **Receptionist Phase**: Query decomposition and planning (shown in workflow visualization)
2. **Librarian Phase**: Intelligent schema selection with lazy loading (Image 2)
3. **Engineer Phase**: SQL generation with focused context (shown in workflow)
4. **Inspector Phase**: Data validation and natural language output (Image 3)

The system provides **full transparency** into the AI decision-making process, showing:
- How medical terminology maps to database schema
- Which columns are selected and why (lazy loading)
- The generated SQL query
- Validation and error correction process
- Final results with human-readable explanations
...
│
├── images/                        # Screenshots and UI documentation
│   ├── WhatsApp Image 2026-02-06 at 10.25.33.jpeg  # Final System Result
│   ├── WhatsApp Image 2026-02-06 at 10.36.56.jpeg  # Librarian Selection
│   ├── WhatsApp Image 2026-02-06 at 10.37.29.jpeg  # Inspector Output
│   └── WhatsApp Image 2026-02-06 at 11.23.18.jpeg  # System Health
│
